...
export default defineComponent({
  ...
  methods: {
    ...
    getUser() {
      axios.post(`/user/show/${this.user_id}`, {}, {
      ...
      .then((res) => {
        ...
        this.AddEditformState.role_id = this.role_id = data.role_id
        this.AddEditformState.role = this.role = data.role
    ...
    submit() {
      ...
      axios.post(`/user/update/${this.user_id}`, toRaw(this.AddEditformState), {
        headers: {"Authorization" : `Bearer ${this.$cookies.get('igo-x-access-token')}`}
      })
      .then((res) => {
