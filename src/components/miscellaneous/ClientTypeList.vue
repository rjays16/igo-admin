    ...
    <a-row :gutter="24">
      <a-col :span="6">
        ...
        <a-form-item
          label="Filter"
          name="clientfilter">
          <a-input v-model:value="form.filter_allcolumn" @keypress.enter="searchClientTypes(true)">
            <template #prefix>
        ...
        <div class="pagination text-right">
          <div class="d-inline-block">
            <ul class="pagination-list lst-none p-0 d-inline-flex w-100">
              ...
              <li class="ant-pagination-next mr-2" :class="client_type_pagination.links.next ? '' : 'ant-pagination-disabled'">
                <button class="ant-pagination-item-link" type="button" @click="toNextPage" :disabled="!client_type_pagination.links.next">
                  <CaretRightFilled />
                </button>
              </li>
    ...
    <template #headerCell="{ column }">
      <template v-if="column.key === 'id'">
        <div class="ant-table-column-sorters">
          <span class="ant-table-column-title">Client Type ID<br>
          <input class="single-input" type="text" v-model="form.filter_id" @keypress.enter="searchClientTypes(true)" /></span>
      ...
      <template v-if="column.key === 'client_type'">
        <div class="ant-table-column-sorters">
          ...
          <input class="single-input" type="text" v-model="form.filter_client_type" @keypress.enter="searchClientTypes(true)" /></span>
          <span class="ant-table-column-sorter ant-table-column-sorter-full">
      ...
      <template v-if="column.key === 'description'">
        <div class="ant-table-column-sorters">
          ...
          <input class="single-input" type="text" v-model="form.filter_description" @keypress.enter="searchClientTypes(true)" /></span>
          <span class="ant-table-column-sorter ant-table-column-sorter-full">
    ...
    <template #bodyCell="{ column, record, text }">
      ...
      <template v-if="column.dataIndex === 'action'">
        <div class="action-container">
          ...
          <DeleteData @click="createAuditTrail('click', 'Clicked on the delete client type button.')" :type_id="record.id" :description="record.description" />
        </div>
      </template>
    </template>
  </a-table>
...
import EditData from "../../components/clients/EditClientType.vue";
import DeleteData from "../../components/clients/DeleteClientType.vue";
...
export default defineComponent({
  ...
  setup() {
    const columns = ref([
      ...
      { title: 'Client Type', dataIndex: 'client_type', key: 'client_type', width: 100, resizable: true, },
  ...
  methods: {
    getClientTypes() {
      ...
      this.getClientTypesByParams()
    },
    getClientTypesByParams() {
      this.loading = true
      this.form.export_to = ''

      axios.post("/client-type", this.form, {
        headers: {"Authorization" : `Bearer ${this.$cookies.get('igo-x-access-token')}`}
      })
      .then((res) => {
        let data = res.data
        this.client_type_list = data.data
        this.client_type_pagination = data
        this.loading = false
    ...
    searchClientTypes(create_trail) {
      this.current = 1
    ...
    toFirstPage() {
      this.searchClientTypes()
      this.scrollToTop()

      this.createAuditTrail('click', 'Clicked on the "go to first page" button in the pagination for client types.')
    },
    toPreviousPage() {
      ...
      this.createAuditTrail('click', 'Clicked on the next button in the pagination for client types.')
    },
    toNextPage() {
      this.current = this.current + 1
      this.form.page = this.current
    ...
    toLastPage() {
      this.current = this.client_type_pagination.meta.to
      this.form.page = this.current
      this.getClientTypesByParams()
      this.scrollToTop()

      this.createAuditTrail('click', 'Clicked on the "go to last page" button in the pagination for client types.')
    },
    ...
    showEditDataModal(type_id) {
      ...
      this.createAuditTrail('click', 'Clicked on the edit client type button.')
    },
    createAuditTrail(type, activity_name = null) {
      ...
      if(type == 'click') {
        ...
        let search_string = 'Searched client types by '
