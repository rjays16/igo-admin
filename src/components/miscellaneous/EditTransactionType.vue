<template>
    <a-modal centered v-model:visible="visible" ok-text="Update" title="Update Transaction Type" @ok="handleOk">
        <a-form
...
export default defineComponent({
    ...
    methods: {
        ...
        getTransactionType() {
            axios.post(`/transaction-type/show/${this.type_id}`, {}, {
                headers: {"Authorization" : `Bearer ${this.$cookies.get('igo-x-access-token')}`}
            })
            .then((res) => {
                let data = res.data.data
                this.addTransactionTypeFormState.trans_type = data.trans_type
                this.addTransactionTypeFormState.description = data.description
            })
            .catch((err) => {
                console.error(err)
        ...
        submit() {
            ...
            axios.post(`/transaction-type/update/${this.type_id}`, toRaw(this.addTransactionTypeFormState), {
                headers: {"Authorization" : `Bearer ${this.$cookies.get('igo-x-access-token')}`}
            })
            .then((res) => {
                ...
                this.visible = false
                this.loading = false
                this.$parent.getTransactionTypes()
            })
            .catch((err) => {
                console.error(err)
                message.error(err.response.data.message)
